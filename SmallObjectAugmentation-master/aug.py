...
def copysmallobjects1(image_dir, label_dir, save_pic, save_txt):
    image = cv2.imread(image_dir)

    labels = read_label_txt(label_dir)
    if len(labels) == 0: return

    # 获取yolo的txt文件格式的标签，第一列为 目标的 id值
    ...
    check_dir(save_pic)
    # print('image',image)
    yolo_txt_dir = join(save_txt, basename(image_dir.replace('.jpg', '_augment.txt')))
    # cv2.imwrite(join(save_dir, basename(image_dir).replace('.jpg', '_augment.jpg')), image)
    print(join(save_pic, basename(image_dir.replace('.jpg', '_augment.jpg'))))
    cv2.imwrite(join(save_pic, basename(image_dir.replace('.jpg', '_augment.jpg'))), image)
    convert_all_boxes(image.shape, all_boxes, yolo_txt_dir)
...
def copysmallobjects2(image_dir, label_dir, save_pic, save_txt, small_img_dir, cl_id):
    image = cv2.imread(image_dir)
    labels = read_label_txt(label_dir)
    if len(labels) == 0:  #
        return
    ...
    for small_img_dirs in small_img_dir:
        ...
        new_bboxes = random_add_patches2(roi.shape, rescale_labels, image.shape, paste_number=1, iou_thresh=0, cl_id=cl_id)
        count = 0
        for new_bbox in new_bboxes:
            count += 1
